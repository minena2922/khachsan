...
<script>
    ...
    function saveReview() {
        var ma_kh = "<?php echo $ma_kh; ?>";
        var ma_loai = document.getElementById("map").value;
                ...
                if (xhr.status == 200) {
                    try {
                        ...
                        if (response.status === "success") {
                            ...
                            closeReviewPopup();
                            if (response.ten_loai) {
                                window.location.href = 'xemphong.php?loai_phong=' + encodeURIComponent(response.ten_loai);
                            } else {
                                console.error("Tên loại phòng không được trả về từ server.");
                                alert("Đánh giá đã được lưu, nhưng không thể chuyển hướng đến trang xem phòng.");
                                window.location.reload(); // Tải lại trang hiện tại
                            }
                        } else {
                            alert("Lỗi khi lưu đánh giá: " + (response.message || "Không xác định"));
                        }
                    } catch (e) {
        ...
        xhr.send("ma_kh=" + encodeURIComponent(ma_kh) + "&ma_loai=" + encodeURIComponent(ma_loai) + "&noi_dung=" + encodeURIComponent(noi_dung) + "&ngay_danh_gia=" + encodeURIComponent(ngay_danh_gia));
    }
