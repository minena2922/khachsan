...
<body>
    ...
    <?php
    $loai_phong = isset($_GET['loai_phong']) ? $conn->real_escape_string($_GET['loai_phong']) : '';
    if (!empty($loai_phong)) {
        $sql_comment = "SELECT dg.*, kh.emailkh, lp.ten_loai
                        FROM danh_gia dg
                        JOIN khach_hang kh ON kh.ma_kh = dg.ma_kh 
                        JOIN loai_phong lp ON lp.ma_loai = dg.ma_loai 
                        WHERE lp.ten_loai = ?
                        ORDER BY dg.ngay_danh_gia DESC";
        
        if ($stmt = $conn->prepare($sql_comment)) {
            $stmt->bind_param("s", $loai_phong);
            $stmt->execute();
            ...
            if ($result_comment->num_rows > 0) {
                while ($row_comment = $result_comment->fetch_assoc()) {
                    ...
                    <div class="danhgia">
                        ...
                        <span><?php echo date('d/m/Y', strtotime($row_comment['ngay_danh_gia'])); ?></span>
                        <p><?php echo htmlspecialchars($row_comment['noi_dung']); ?></p>
                    </div>
                    <?php
                }
            } else {
                echo "<p>Không có đánh giá nào cho loại phòng này.</p>";
            }
            $stmt->close();
        } else {
            echo "<p>Lỗi trong quá trình chuẩn bị truy vấn: " . $conn->error . "</p>";
        }
    } else {
        echo "<p>Không có thông tin về loại phòng.</p>";
    }
    ?>
</div>
